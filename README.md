...
solvent/
data/{raw,processed}/ # raw is sacred, never overwritten by hand
src/
config/ # settings each component needs before running
entity/ # config + artifact dataclasses — the "receipts"
components/ # the real, reusable, independently-testable logic
...
*(Instructions will fill out as components come online — this section will stay honest, not aspirational.)*

---

## Notes

Built end-to-end from scratch — no scaffolding generators, no boilerplate copy-paste.
Every component is read about first, then implemented and tested by hand.
Bugs found along the way (like a duplicate-logging-handler issue on Day 1)
are kept as commits, not squashed away — the messy process is part of the point.